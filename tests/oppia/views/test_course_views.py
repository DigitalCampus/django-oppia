...
class CourseViewsTest(OppiaTestCase):
    ...
    def test_tag_view(self):
        ...
        url = reverse('oppia_tag_courses', args=[2])
    ...
    def test_tag_view_get_page_1(self):
        self.client.force_login(user=self.admin_user)
        url = '%s?page=1' % reverse('oppia_tag_courses', args=[2])
        response = self.client.get(url)
        self.assertEqual(200, response.status_code)
    ...
    def test_tag_view_get_page_9999(self):
        self.client.force_login(user=self.admin_user)
        url = '%s?page=9999' % reverse('oppia_tag_courses', args=[2])
        response = self.client.get(url)
    ...
    def test_tag_view_get_page_abc(self):
        self.client.force_login(user=self.admin_user)
        url = '%s?page=abc' % reverse('oppia_tag_courses', args=[2])
        response = self.client.get(url)
