...
EMBED_TEMPLATE = "[[media object='{\"filename\":\"%s\", \
                  \"download_url\":\"%s\", \
                  \"digest\":\"%s\", \
                  \"filesize\":%d, \
                  \"length\":%d}']]IMAGE/TEXT HERE[[/media]]"
...
class UploadedMedia(models.Model):
    ...
    def get_default_image(self):
        try:
            img = UploadedMediaImage.objects.get(uploaded_media=self,
                                                 default_image=True)
            return img
        except UploadedMediaImage.DoesNotExist:
            try:
                img = UploadedMediaImage.objects \
                    .filter(uploaded_media=self) \
...
class UploadedMediaImage(models.Model):
    ...
    uploaded_media = models.ForeignKey(UploadedMedia,
