...
class UploadTrackerForm(forms.Form):  
    tracker_file = forms.FileField(
    ...
    def __init__(self, *args, **kwargs):
        super(UploadTrackerForm, self).__init__(*args, **kwargs)
        self.helper = FormHelper()
        self.helper.form_action = reverse('oppia_tracker_upload')
    ...
    def clean(self):
        cleaned_data = super(UploadTrackerForm, self).clean()
        tracker_file = cleaned_data.get("tracker_file")
        
        if tracker_file is not None:
            if tracker_file.content_type not in settings.OPPIA_UPLOAD_TRACKER_FILE_TYPES:
