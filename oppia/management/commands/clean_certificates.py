...
class Command(BaseCommand):
    help = 'Cleans certificate files - i.e. removes old certificate files that are no longer valid'
    ...
    def handle(self, *args, **options):
        ...
        for root, dirs, files in os.walk(cert_dir):
            for name in files:
                ...
                relative_path = full_path.replace(os.path.join(settings.MEDIA_ROOT,''), '')
                try:
